...
class App extends Component {
  ...
  bhanu = ids => {
    this.setState({tabs: ids.tabId})
  }

  desc = () => {
    this.setState({active: true, score: 0, seconds: 60, image: imagesList[0].imageUrl,
    tabs: tabsList[0].tabId,
    ids: imagesList[0].id})
  }

  filtering = tabs => {
    const updated = imagesList.filter(each => each.category === tabs)
    return updated
  }

  render() {
    this.intervalId = null
    const {score, seconds, image, tabs, active} = this.state
    const filters = this.filtering(tabs)
    if (active === false) {
      clearInterval(this.intervalId)
      return (
        <div className="main1">
          <ul className="buffalo">
          <li className="office">
            <img
          ...
          <li className="cow">
            ...
            <div className="curd">
              <img
                ...
                className="image2"
              />
              <p className="para2">{seconds} sec</p>
            </div>
          </li>
        </ul>
          <div className="donkey">
            ...
            <button className="but3" type="button" onClick={this.desc}>
              <img
    ...
    return (
      <div className="main">
        ...
        <div className="tiger">
          <img src={image} alt="match" className="image3" />
        </div>
        <ul className="lion">
          {tabsList.map(each => {
            const actived = each.tabId === tabs ? 'baby' : ''
            return (
              <li className="dirty" key={each.tabId}>
                <button
                  className={`but ${actived}`}
                  type="button"
                  onClick={() => this.bhanu(each)}
                >
                  {each.displayText}
                </button>
              </li>
        ...
        <ul className="icon">
          {filters.map(each => (
            <li className="tadaka" key={each.id}>
              <button
                className="but"
                type="button"
                onClick={() => this.testing(each)}
              >
                <img src={each.thumbnailUrl} alt="thumbnail" className="image5" />
              </button>
            </li>
          ))}
        </ul>
